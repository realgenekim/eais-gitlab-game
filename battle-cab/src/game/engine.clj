...
(defn add-player!
  ...
  (let [state @(:game-state sys)
    ...
    (let [[new-state creds] (core/add-player state player-name)]
      (if (:reconnected creds)
        ;; Reconnecting — return existing creds, no state change
        (do
          (log/info :player-reconnected :name player-name :id (:id creds))
          (assoc creds :gear-catalog (core/available-gear state)))
        ;; New player
